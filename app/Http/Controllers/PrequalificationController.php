...
class PrequalificationController extends Controller
{
    function index(){
        ...
        $session = SESSION::where('SESSION_USER_ID',Auth::user()->MEMBER_UUID)->where('SESSION_SURVEY_UUID','DE65536C-AF1F-43BF-9362-5C8662002C8C')->get();
    ...
    function store(){
        ...
        $crypt_email = $CryptController->cryptString(Auth::user()->MEMBER_EMAIL);
        $crypt_last_login = $CryptController->cryptString(Auth::user()->MEMBER_LAST_LOGIN);
        $survey_slug = DB::Connection('SURVEY')->table('SURVEY')->select('SURVEY_SLUG')->where('SURVEY_UUID','DE65536C-AF1F-43BF-9362-5C8662002C8C')->first();
